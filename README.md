...
MCPサーバーは「リコリス・リコイル」のクルミ（ウォールナット）を想定した投稿ペルソナをクライアントへ提供します。本プロジェクトは個人制作の非公式ファンプロジェクトであり、作品の権利者や関係各社とは関係ありません。作品名・キャラクター名などの権利は各権利者に帰属します。
...
このリポジトリのオリジナルコードは[MIT License](LICENSE)で公開します。作品名・キャラクター名など、第三者が保有する権利はライセンス対象に含まれません。
